...
namespace area_server.Controllers
{
    public class LoginController : Controller
    {
        ...
        {
            dynamic json = JsonConvert.DeserializeObject("{\"user_id_token\": null, \"user_name\": null, \"user_email\": null}");
            using (var reader = new StreamReader(Request.Body))
            {
                var body = reader.ReadToEnd();
                var dict = HttpUtility.ParseQueryString(body);
                json.user_id_token = dict["user_id_token"];
                json.user_name = dict["user_name"];
                json.user_email = dict["user_email"];
            ...
            if (result.Count == 1)
            {
                ...
                Response.Cookies.Append("user_id", Convert.ToString(result[0].id), option);
            ...
            {
                int inn = DBConnect.my_insert_return("users", "login, email, isOAuth, isValidated", "'" + json.user_name + "', '" + json.user_email + "', 1, 1");
                ...
                DBConnect.my_insert("user_access_token", "user_id", Convert.ToString(inn)); // result_back[0].id));
